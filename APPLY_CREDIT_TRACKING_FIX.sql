-- URGENT: Apply this SQL in your Supabase SQL Editor to fix credit tracking
-- This will update the deduct_credits function to properly log transactions

-- Step 1: Update the deduct_credits function to include transaction logging
...
BEGIN
    ...
    -- THIS IS THE KEY FIX: Log the transaction in credit_transactions table
...
-- Step 2: Grant execute permissions to authenticated users
GRANT EXECUTE ON FUNCTION deduct_credits(INTEGER, TEXT, JSONB) TO authenticated;

-- Step 3: Verify the fix by checking if the function was updated
-- You can run this query to see the function definition:
-- SELECT routine_definition FROM information_schema.routines WHERE routine_name = 'deduct_credits';

-- After running this SQL:
-- 1. Go back to your app and verify another email
-- 2. Check the Daily Credit Usage chart - it should now show the credit usage
-- 3. Check the credit_transactions table - it should have the transaction record